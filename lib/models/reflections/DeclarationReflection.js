...
var ReflectionType_1 = require("../types/ReflectionType");
var DeclarationReflection = (function (_super) {
    ...
    DeclarationReflection.prototype.traverse = function (callback) {
        if (this.typeParameters) {
            this.typeParameters.forEach(function (parameter) { return callback(parameter, Reflection_1.TraverseProperty.TypeParameter); });
        }
        if (this.type instanceof ReflectionType_1.ReflectionType) {
            callback(this.type.declaration, Reflection_1.TraverseProperty.TypeLiteral);
        }
        if (this.signatures) {
            this.signatures.forEach(function (signature) { return callback(signature, Reflection_1.TraverseProperty.Signatures); });
        }
        if (this.indexSignature) {
            callback(this.indexSignature, Reflection_1.TraverseProperty.IndexSignature);
        }
        if (this.getSignature) {
            callback(this.getSignature, Reflection_1.TraverseProperty.GetSignature);
        }
        if (this.setSignature) {
            callback(this.setSignature, Reflection_1.TraverseProperty.SetSignature);
        }
