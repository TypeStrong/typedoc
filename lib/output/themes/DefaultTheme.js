...
var DefaultTheme = (function (_super) {
    ...
    function DefaultTheme(renderer, basePath) {
        ...
        renderer.on(Renderer_1.Renderer.EVENT_BEGIN, this.onRendererBegin, this, 1024);
    }
    ...
    DefaultTheme.prototype.getParameters = function () {
        return [{
                ...
                type: Options_1.ParameterType.String
            }];
    };
    ...
    DefaultTheme.prototype.getNavigation = function (project) {
        ...
        function includeDedicatedUrls(reflection, item) {
            (function walk(reflection) {
                for (var key in reflection.children) {
                    ...
                    if (child.hasOwnDocument && !child.kindOf(Reflection_1.ReflectionKind.SomeModule)) {
        ...
        function buildChildren(reflection, parent) {
            var modules = reflection.getChildrenByKind(Reflection_1.ReflectionKind.SomeModule);
            modules.sort(function (a, b) {
                return a.getFullName() < b.getFullName() ? -1 : 1;
            });
        ...
        function build(hasSeparateGlobals) {
            ...
            var modules = [];
            project.getReflectionsByKind(Reflection_1.ReflectionKind.SomeModule).forEach(function (someModule) {
                var target = someModule.parent;
                var inScope = (someModule == entryPoint);
                while (target) {
                    if (target.kindOf(Reflection_1.ReflectionKind.ExternalModule))
                        return;
                    if (entryPoint == target)
                        inScope = true;
                    target = target.parent;
                }
                if (inScope && someModule instanceof DeclarationReflection_1.DeclarationReflection) {
            ...
            if (modules.length < 10) {
                ...
                buildGroups(entryPoint.getChildrenByKind(Reflection_1.ReflectionKind.SomeModule), root, buildChildren);
            }
            return root;
        }
        var entryPoint = this.getEntryPoint(project);
    ...
    DefaultTheme.prototype.onRendererBegin = function (event) {
        ...
        for (var id in event.project.reflections) {
            ...
            if (reflection instanceof ContainerReflection_1.ContainerReflection && reflection['groups']) {
    ...
    DefaultTheme.getUrl = function (reflection, relative, separator) {
        ...
        if (reflection.parent && reflection.parent != relative &&
            !(reflection.parent instanceof ProjectReflection_1.ProjectReflection))
    ...
    DefaultTheme.applyReflectionClasses = function (reflection) {
        var classes = [];
        if (reflection.kind == Reflection_1.ReflectionKind.Accessor) {
            ...
            var kind = Reflection_1.ReflectionKind[reflection.kind];
            classes.push(DefaultTheme.toStyleClass('tsd-kind-' + kind));
        }
        if (reflection.parent && reflection.parent instanceof DeclarationReflection_1.DeclarationReflection) {
            kind = Reflection_1.ReflectionKind[reflection.parent.kind];
    ...
    DefaultTheme.MAPPINGS = [{
            kind: [Reflection_1.ReflectionKind.Class],
            isLeaf: false,
            directory: 'classes',
            template: 'reflection.hbs'
        }, {
            kind: [Reflection_1.ReflectionKind.Interface],
            isLeaf: false,
            directory: 'interfaces',
            template: 'reflection.hbs'
        }, {
            kind: [Reflection_1.ReflectionKind.Enum],
            isLeaf: false,
            directory: 'enums',
            template: 'reflection.hbs'
        }, {
            kind: [Reflection_1.ReflectionKind.Module, Reflection_1.ReflectionKind.ExternalModule],
            isLeaf: false,
            directory: 'modules',
            template: 'reflection.hbs'
        }];
