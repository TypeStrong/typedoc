...
var declaration_2 = require('../models/reflections/declaration');
var tag_1 = require('../models/comments/tag');
var Converter = (function (_super) {
    ...
    Converter.prototype.convertNode = function (context, node) {
        ...
        if (result && comment != null && comment.indexOf('@notSupportedIn') != -1) {
            ...
            result.comment.tags.push(new tag_1.CommentTag('not supported in', '', tagValue));
            result.notSupportedIn = tag[1].split(/,\s?/);
        }
        if (result && comment != null && comment.indexOf('@componentOptions') != -1) {
            result.setFlag(abstract_1.ReflectionFlag.CoveoComponentOptions, true);
        }
        if (result && result instanceof declaration_2.DeclarationReflection) {
            ...
            if (declarationReflection.extendedTypes) {
                declarationReflection.extendedTypes.forEach(function (type) {
                    if (type.toString().toLowerCase() == 'component') {
                        result.kind = abstract_1.ReflectionKind.CoveoComponent;
                    }
                });
            }
