...
function visitVariableDeclaration(context, node) {
    ...
    context.withScope(variable, function () {
        ...
        if (variable.kind == kind || variable.kind == Reflection_1.ReflectionKind.Event) {
            if (isBindingPattern) {
                variable.type = convertDestructuringType(context, node.name);
            }
            else {
...
function visitConstructor(context, node) {
    ...
    context.withScope(method, function () {
        if (!hasBody || !method.signatures) {
            ...
            var signature = createSignature(context, node, name, Reflection_1.ReflectionKind.ConstructorSignature);
...
function visitFunctionDeclaration(context, node) {
    ...
    context.withScope(method, function () {
        if (!hasBody || !method.signatures) {
            var signature = createSignature(context, node, method.name, Reflection_1.ReflectionKind.CallSignature);
...
function visitCallSignatureDeclaration(context, node) {
    ...
    if (scope instanceof DeclarationReflection_1.DeclarationReflection) {
        ...
        var signature = createSignature(context, node, name, Reflection_1.ReflectionKind.CallSignature);
        if (!scope.signatures)
            scope.signatures = [];
...
function visitIndexSignatureDeclaration(context, node) {
    var scope = context.scope;
    if (scope instanceof DeclarationReflection_1.DeclarationReflection) {
        scope.indexSignature = createSignature(context, node, '__index', Reflection_1.ReflectionKind.IndexSignature);
    }
    return scope;
}
function visitGetAccessorDeclaration(context, node) {
    ...
    context.withScope(accessor, function () {
        accessor.getSignature = createSignature(context, node, '__get', Reflection_1.ReflectionKind.GetSignature);
    });
    return accessor;
}
function visitSetAccessorDeclaration(context, node) {
    var accessor = factories_1.createDeclaration(context, node, Reflection_1.ReflectionKind.Accessor);
    context.withScope(accessor, function () {
        accessor.setSignature = createSignature(context, node, '__set', Reflection_1.ReflectionKind.SetSignature);
    });
    return accessor;
}
