...
var Repository = (function () {
    function Repository(path) {
        ...
        if (out.code == 0) {
            out.output.split('\n').forEach(function (file) {
                if (file != '') {
                    _this.files.push(BasePath_1.BasePath.normalize(path + '/' + file));
                }
            });
        }
    ...
    Repository.tryCreateRepository = function (path) {
        ...
        if (out.code != 0)
            return null;
        return new Repository(BasePath_1.BasePath.normalize(out.output.replace("\n", '')));
    };
    return Repository;
})();
...
Converter_1.Converter.registerPlugin('gitHub', GitHubPlugin);
