...
export class OptionsParser
{
     ...
     */
    addParameter(parameters:IParameter[]):OptionsParser;

    /**
     * @param rest One or multiple parameter definitions that should be registered.
     */
    addParameter(...rest:IParameter[]):OptionsParser;

    /**
     * Register one or multiple parameter definitions.
     */
    addParameter(...args:any[]):OptionsParser {
        args.forEach((param) => {
    ...
    setOption(param:IParameter, value?:any):boolean {
        if (param.isArray && Util.isArray(value)) {
            var result = true;
            value.forEach((value:any) => result = this.setOption(param, value) && result);
            return result;
        }
