 ...
 */
function convertTypeLiteral(context:Context, symbol:ts.Symbol, node?:ts.Node):ReflectionType {
 ...
 */
function convertTypeAliasNode(node:ts.TypeReferenceNode):ReferenceType {
 ...
 */
function convertStringLiteralExpression(node:ts.StringLiteral):StringLiteralType {
 ...
 */
function convertArrayTypeNode(context:Context, node:ts.ArrayTypeNode):Type {
 ...
 */
function convertTupleTypeNode(context:Context, node:ts.TupleTypeNode):TupleType {
    var elements:Type[];
 ...
 */
function convertUnionTypeNode(context:Context, node:ts.UnionTypeNode):UnionType {
    var types:Type[] = [];
    if (node.types) {
 ...
 */
function convertTypeParameterNode(context:Context, node:ts.TypeReferenceNode):Type {
    if (node.typeName) {
        var name = ts.getTextOfNode(node.typeName);
 ...
 */
function convertTypeReferenceNode(context:Context, node:ts.TypeReferenceNode, type:ts.TypeReference):Type {
    if (!type.symbol) {
 ...
 */
function convertIntrinsicType(type:ts.IntrinsicType):IntrinsicType {
 ...
 */
function convertStringLiteralType(type:ts.StringLiteralType):StringLiteralType {
 ...
 */
function convertEnumType(context:Context, type:ts.Type):Type {
 ...
 */
function convertTupleType(context:Context, type:ts.TupleType):TupleType {
    var elements:Type[];
 ...
 */
function convertUnionType(context:Context, type:ts.UnionType):UnionType {
    var types:Type[];
    if (type && type.types) {
 ...
 */
function convertTypeReferenceType(context:Context, type:ts.TypeReference):Type {
    if (!type.symbol) {
        return new IntrinsicType('Object');
    } else if (type.symbol.flags & ts.SymbolFlags.TypeLiteral || type.symbol.flags & ts.SymbolFlags.ObjectLiteral) {
 ...
 */
export function convertDestructuringType(context:Context, node:ts.BindingPattern):Type {
