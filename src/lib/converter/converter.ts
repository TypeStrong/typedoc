...
> {
    ...
    convert(
        ...
        this.compile(entryPoints, context, packages);
    ...
    private compile(
        ...
        for (const packageProgram of packages) {
            // This should not be undefined as we're just pulling
            // the sourceFile from its associated program
            const sourceFile = packageProgram.program.getSourceFile(
                packageProgram.entryPoint
            )!;
            entries.push({
                entryName: packageProgram.packageName,
                file: packageProgram.entryPoint,
                sourceFile,
